...
VDPA_BIN_FILE="/usr/bin/vdpa"

# Give help text for parameters.
usage()
{
    /bin/echo -e "This is an entrypoint script for vDPA CNI to overlay its"
    ...
    /bin/echo -e "\t--vdpa-bin-file=$VDPA_BIN_FILE"
}
...
for i in $CNI_BIN_DIR $VDPA_BIN_FILE
do
  if [ ! -e "$i" ]; then
...
cp -f $VDPA_BIN_FILE $CNI_BIN_DIR
