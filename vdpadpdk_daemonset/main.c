...
static
void vdpa_get_socketpath(char *pPCIAddr, char *pSocketpath)
{
	struct rte_pci_addr addr;
	int i;

	pSocketpath[0] = '\0';
	if (eal_parse_pci_DomBDF(pPCIAddr, &addr) == 0) {
		for (i = 0; i < RTE_MIN(MAX_VDPA_SAMPLE_PORTS, dev_total); i++) {
			if (rte_pci_addr_cmp(&addr, &vports[i].addr) == 0) {
				rte_strscpy(pSocketpath, vports[i].ifname, MAX_PATH_LEN);
				break;
			}
		}
	}
...
static
void vdpa_dump_table(void)
{
	int i;

	printf("Detected vDPA Interfaces:\n");
	for (i = 0; i < RTE_MIN(MAX_VDPA_SAMPLE_PORTS, dev_total); i++) {
		printf("  DPDK Index: %d VID: %d PCI: " PCI_PRI_FMT " Socketpath: %s\n",
				vports[i].did,
				vports[i].vid,
...
static
void tst_lookup(void)
{
	char pciAddrStr[MAX_PATH_LEN];
	char socketPathStr[MAX_PATH_LEN];

	rte_strscpy(pciAddrStr, "0000:82:01.1", MAX_PATH_LEN);
	vdpa_get_socketpath(pciAddrStr, socketPathStr);
	printf("Test: %s - Found: %s\n", pciAddrStr, socketPathStr);

	rte_strscpy(pciAddrStr, "0000:85:01.1", MAX_PATH_LEN);
	vdpa_get_socketpath(pciAddrStr, socketPathStr);
	printf("Test: %s - Found: %s\n", pciAddrStr, socketPathStr);
		...
		__attribute__((unused)) void *data)
{
	...
	vports[devcnt].addr = addr.pci_addr;
...
main(int argc, char *argv[])
{
	...
	if (interactive == 1) {
		...
		for (i = 0; i < RTE_MIN(MAX_VDPA_SAMPLE_PORTS, dev_total);
				i++) {
			struct rte_vdpa_device *vdev = rte_vdpa_get_device(i);
			if (vdev)
				vports[i].addr = vdev->addr.pci_addr;
			vports[i].did = i;
