...
var (
	scyllaDpdkDynamicFile = flag.String("filename", "/var/run/scylla/scylla_dpdk_dynamic.conf", "Name of file to write data too.")
)
...
func main() {
	...
	for index, iface := range ifaceResponse.Interface {
		...
		case types.INTERFACE_TYPE_VHOST:
			if iface.Vhost != nil {
				tmpStr = fmt.Sprintf("--vdev=net_virtio_user%d,path=%s", vhostCnt, iface.Vhost.Socketpath)
				vhostCnt++
			}
